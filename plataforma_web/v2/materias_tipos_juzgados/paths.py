...
from plataforma_web.v2.materias_tipos_juzgados.crud import (
    get_materias_tipos_juzgados,
    get_materia_tipo_juzgado,
)
...
async def detalle_materia_tipo_juzgado(
    ...
    try:
        materia_tipo_juzgado = get_materia_tipo_juzgado(db, materia_tipo_juzgado_id)
    except IndexError as error:
        raise HTTPException(status_code=404, detail=f"Not found: {str(error)}") from error
    except ValueError as error:
        raise HTTPException(status_code=406, detail=f"Not acceptable: {str(error)}") from error
