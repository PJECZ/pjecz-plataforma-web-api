...
def get_materias_tipos_juzgados(db: Session, materia_id: int):
    ...
    return db.query(MateriaTipoJuzgado, Materia).join(Materia).filter(MateriaTipoJuzgado.materia == materia).filter(MateriaTipoJuzgado.estatus == "A").order_by(MateriaTipoJuzgado.clave).all()
